...
export default function SignUp() {
  ...
  const { signUp } = useAuth(); // Ensure this works correctly
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If `signUp()` is needed for auth state management, call it
      ...
      <div className="max-w-md w-full space-y-8">
        ...
        <motion.form initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5, delay: 0.2 }} className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <p className="text-center text-sm text-gray-400">
            ...
            <Link to="/signin" className="font-medium text-pink-500 hover:text-pink-400">
              Sign in
            </Link>
          </p>
        </motion.form>
